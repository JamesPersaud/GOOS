...
namespace GOOS.JFX.Core
{
    ...
    public class QuakeCamera
    {
        ...
        public void Update(MouseState currentMouseState, KeyboardState keyState, GamePadState gamePadState)
        {
			...
			if (InDeathThroes)
			{
				leftrightRot -= 0.025f; // spin around
				if (CurrentDeath > DeathFloor)
					CurrentDeath -= 0.1f;
				cameraPosition.Y = CurrentDeath;
				UpdateViewMatrix();
			}
			else
			{
        ...
        {
			//Check the boundaries of camera up/down rotation.
			//
			// 1.4 radians is approx 80 degrees
			//
			// This stops the player being able to double over backwards or put his head between his legs
			//
			// TODO: Make switchable so that this camera can be used for a flight sim game.
			//
			if (updownRot > 1.4f)			
				updownRot = 1.4f;
			if (updownRot < -1.4f)
		...
		/// Kill the player
		/// </summary>
		public void Die(float floor)
		{
			InDeathThroes = true;
			DeathFloor = floor;
			CurrentDeath = cameraPosition.Y;
		}
    }
}
